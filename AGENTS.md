# AGENTS.md — Desktop Intelligence: AI Orchestration Layer
...
## 0. CHANGE HISTORY LOG (MANDATORY — READ BEFORE EVERY SESSION)

Your persistent memory lives in **[progress.md](./progress.md)** at the root of the repository.

**Before touching a single line of code, read every row in progress.md.**
It contains the complete ordered history of every change ever made to this project.
Violating this rule causes regressions — changes have been undone before because this was skipped.

### What to do each session

1. **Read** `progress.md` from top to bottom before starting any work.
2. **Work** — implement the feature or fix as requested.
3. **Append** a new row to the bottom of `progress.md` when done.
4. **Bump** `package.json` version and commit (Karan pushes manually — never push).

### Row format (append to progress.md, never edit existing rows)

```
| {next_number} | {YYYY-MM-DD} | {file(s) changed} | {what was done — be specific} | ✅ Done |
```

Status values: `✅ Done` · `🔴 OPEN` · `🟡 Minor` · `🚧 In Progress` · `❌ Abandoned`

If you abandon an approach, add a row marked `❌ Abandoned` with the reason before trying a new one.
...
├── AGENTS.md                     — THIS FILE
└── CLAUDE.md                     — copy of AGENTS.md for Claude
```
...
## 12. WORKING PRINCIPLES FOR AGENTS
...
*Last updated: 2026-03-27 | Version: 1.2 | Maintained by: Agents (update this on every session)*