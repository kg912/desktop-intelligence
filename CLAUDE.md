...
| 91 | 2026-04-09 | src/main/services/ChatService.ts | Gemma 4 follow-up fixes: (1) Issue 2 — `<think>` block leaking to renderer on mid-stream tool call interception: added `thinkStripPattern` + `visibleCleaned` (`patchedCleaned` with all complete `<think>…</think>` blocks stripped via regex) in the `detectMidStreamToolCall` interception path; changed `CHAT_STREAM_RETRACT` send to use `visibleCleaned` instead of the old `retractedClean` (which only cut at thinkStart but left post-think text out); `patchedCleaned` (with think block) still used for `currentMessages` so model retains prior reasoning. Also updated `stripLeadingThinkClose` to strip a complete leading `<think>…</think>` block (third `.replace` line) — handles Gemma inline first-chunk emission. (2) Issue 3 — `<tool_call|>` token leaking as visible text: added `'<tool_call|>'` and `'<|tool_call>'` to `STOP_SEQUENCES` so LM Studio halts generation on Gemma pipe-delimiter tokens before they reach the stream; primary fix. | ✅ Done |
| 90 | 2026-04-09 | src/main/services/ChatService.ts, src/main/services/__tests__/ChatService.test.ts | Gemma 4 hardening — three-bug fix pass: (1) Bug 1 — Step 1 JSON truncation: added `extractFirstValidJSON()` helper (exported) that tries JSON.parse directly, falls back to brace-depth scanning to extract the first balanced {...} block (recovers from trailing `<tool_call|>}<eos>` noise), then uses regex as last resort when truncated at max_tokens boundary. Replaced bare `JSON.parse(rawDecision)` with this helper throughout. Also raised `step1Body.max_tokens` 250→400 — Gemma 4's think block eats tokens before JSON starts. (2) Bug 2 — New [TOOL_REQUEST]...[END_TOOL_REQUEST] format: added Format G to `parseRawToolCall` (checked before all other formats; handles JSON `{name, arguments}` inside tags). Added Case 7 (closed tag) and Case 8 (unclosed tag with complete JSON) to `detectMidStreamToolCall`. Added `[TOOL_REQUEST]...[END_TOOL_REQUEST]` to the banned formats list in `WEB_SEARCH_SYSTEM_ADDENDUM`. Exported `parseRawToolCall` and `detectMidStreamToolCall` for unit testing. (3) Bug 3 — Gemma reasoning hell: (A) Added Gemma-specific thinking budget cap in `step2ThinkingField` — when fast mode AND Gemma, use `thinking:{type:'enabled', budget_tokens: 1024/2048}` instead of `thinking:{type:'disabled'}` since Gemma ignores the disabled field. (B) Added explicit comment confirming Step 1 decision system message never contains `<|think|>`. (C) Added Gemma fast path inside `if (shouldAttemptSearch)` — Gemma bypasses Step 1 entirely, extracts user message text directly as search query (trimmed to 120 chars at word boundary), skipping the 8–15s JSON decision round. Non-Gemma path unchanged inside `else` branch. Added `isGemmaModel` flag before `try` block. New unit tests: 7 for `extractFirstValidJSON`, 5 for `parseRawToolCall` Format G, 4 for `detectMidStreamToolCall` Format G. All existing tests preserved. | ✅ Done |
