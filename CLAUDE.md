...
| 16 | TBD | src/renderer/src/components/layout/InputBar.tsx, store/ModelStore.tsx | **P1 OPEN:** Thinking vs Fast mode toggle not yet implemented in UI. Section 5 spec: toggle near input bar, default Fast, auto-switch to Thinking for PDF/image, add mode divider in chat | 🟡 OPEN |
| 17 | TBD | src/renderer/src/components/ErrorBanner.tsx (or equivalent) | **P1 OPEN:** Stale error banner — never clears after daemon recovers. FIX: dispatch clear-error on first successful health check | 🟡 OPEN |
