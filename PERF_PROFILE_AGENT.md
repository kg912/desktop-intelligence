# PERF_PROFILE_AGENT.md — Live Performance Profiling Agent
## Claude Code Sub-Agent Spec · Desktop Intelligence

> FULLY AUTONOMOUS. No readline prompts. No user confirmations.
> READ CLAUDE.md AND PROGRESS.md BEFORE STARTING.
> Do NOT touch any source files during this run.

---

## Phase 1 — Write the profiling script

Install `ws` if not present: `npm install --save-dev ws`
...
## Phase 2 — Launch and run

```bash
# Kill any existing instance on port 9222
pkill -f "DEV.*Desktop Intelligence" 2>/dev/null
sleep 2

# Launch DEV app with remote debugging
open -a "/Applications/[DEV] Desktop Intelligence.app" --args --remote-debugging-port=9222
sleep 5

# Run profiler — clicks suggestion, captures full streaming session
...
Read `scripts/perf-report.json`. Print a structured report:

```
## Profiling Results — Streaming Session

### Setup
- Trigger: [which suggestion was clicked]
- Duration: [how long streaming took]

### Top 20 JS functions by self-time
| Rank | Function | Source | Self-time (ms) |
...

### Top 20 by call frequency (sample count)
| Rank | Function | Samples |
...

### Analysis — map each hot function to a hypothesis

For EACH function in the top 15 by self-time, state:
  a) Which module/component it likely belongs to (React reconciler, Framer Motion,
     Preact signals, MarkdownRenderer, highlight.js, etc.)
  b) Whether it confirms a specific fix from PERF_AGENT.md (Fix 1–5)
  c) Estimated contribution to observed jank

Key things to look for and call out explicitly:
  - Any React reconciler functions (reconcileChildren, renderWithHooks, commitWork)
    appearing at HIGH frequency → confirms Fix 2 (60Hz re-renders)
  - animateVisualElement / wake2 / PresenceChild at HIGH self-time
    → confirms Fix 1 (motion.div per message) at scale
  - highlight / hljs functions → MarkdownRenderer code highlighting cost
  - Any Preact signal internals (batch, notify, compute) → signal overhead
  - requestAnimationFrame callback chains → rAF flood

### Verdict
- Confirmed fixes (list with evidence)
- Unconfirmed fixes (list with reason)
- Any new findings not in PERF_AGENT.md
```

No source edits. Report only.
