...
export function useChat({ chatId = null, onChatCreated }: UseChatOptions = {}) {
  ...
  const [liveToolCall, setLiveToolCall] =
    useState<Message["liveToolCall"]>(null);
  ...
  useEffect(() => {
    ...
    const unsubEnd = window.api.onChatStreamEnd((stats: GenerationStats) => {
      ...
      if (finalToolCall?.phase === "done") {
        ...
        setMessages((prev) => {
          ...
          updated[idx] = {
            ...updated[idx],
            toolCall: {
              query: finalToolCall.query,
              results: finalToolCall.results ?? [],
              formattedContent: finalToolCall.formattedContent ?? "",
            },
          };
          return updated;
        });
        ...
        const mentionsFailure =
          /search.{0,40}(fail|unavailable|unable|error|couldn)/i.test(
            assistantContent,
          );
        if (mentionsFailure) {
      ...
      if (activeChatId && assistantMsgId && assistantContent) {
        const toolCallToSave =
          finalToolCall?.phase === "done"
            ? JSON.stringify({
                query: finalToolCall.query,
                results: finalToolCall.results ?? [],
                formattedContent: finalToolCall.formattedContent ?? "",
    ...
    const unsubSearch = window.api.onWebSearchStatus((s) => {
      if (s.phase === "searching") {
        ...
        patchAssistant({
          isSearching: true,
          liveToolCall: { phase: "searching", query: s.query },
        });
      } else if (s.phase === "done") {
        setIsSearching(false);
        liveToolCallRef.current = {
          phase: "done",
          query: s.query,
          results: s.results ?? [],
          formattedContent: s.formattedContent ?? "",
        };
        setLiveToolCall({
          phase: "done",
          query: s.query,
    ...
    const unsubRetract = window.api.onChatStreamRetract(
      (cleanContent: string) => {
